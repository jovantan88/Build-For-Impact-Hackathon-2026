...
export default function ReviewPage() {
    ...
    const handleUpdateIngredient = (tempId: string, field: "name" | "quantity" | "unit", value: string | number) => {
        setIngredients(ingredients.map((item) => (item.tempId === tempId ? { ...item, [field]: value } : item)));
    };
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold font-heading">Review Items</h1>
                <p className="text-muted-foreground">We found {ingredients.length} ingredients. Edit or remove as needed.</p>
            </div>
            ...
            <Card>
                ...
                <CardContent>
                    {ingredients.length === 0 ? (
                        <p className="text-muted-foreground text-center py-8">No ingredients yet</p>
                    ) : (
                        <div className="space-y-2">
                            {ingredients.map((item) => (
                                <div
                                    key={item.tempId}
                                    className="flex items-center gap-3 p-3 bg-muted/30 border border-border/50 rounded-lg group hover:bg-muted/50 hover:border-border transition-colors"
                                >
                                    {editingId === item.tempId ? (
                                        <>
                                            <Input
                                                value={item.name}
                                                onChange={(e) => handleUpdateIngredient(item.tempId, "name", e.target.value)}
                                                className="flex-1"
                                                autoFocus
                                            />
                                            <Input
                                                type="number"
                                                value={item.quantity || ""}
                                                onChange={(e) => handleUpdateIngredient(item.tempId, "quantity", parseFloat(e.target.value) || 0)}
                                                className="w-20"
                                                placeholder="Qty"
                                            />
                                            <Input
                                                value={item.unit || ""}
                                                onChange={(e) => handleUpdateIngredient(item.tempId, "unit", e.target.value)}
                                                className="w-20"
                                                placeholder="Unit"
                                            />
                                        ...
                                        <>
                                            <div className="flex-1 cursor-pointer" onClick={() => setEditingId(item.tempId)}>
                                            ...
                                            {item.needsExpiryDate && (
                                                <Badge variant="outline" className="text-amber-500/80 border-amber-400/40 bg-amber-500/5">
            ...
            {excludedItems.length > 0 && (
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Non-ingredient items. Click to restore if needed.</CardDescription>
                    </CardHeader>
                    <CardContent>
            ...
            <div className="flex justify-end">
                <Button size="lg" onClick={handleContinue} disabled={ingredients.length === 0}>
                    Continue
