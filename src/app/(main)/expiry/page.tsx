      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm text-gray-600">
          <span>
            {isComplete
              ? "All done!"
              : `Item ${currentIndex + 1} of ${itemsNeedingExpiry.length}`}
          </span>
      ...
      {!isComplete ? (
        <Card className="overflow-hidden">
          ...
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {quickOptions.map((option) => (
                <Button
                  ...
                  variant="outline"
                  className="h-12"
                  onClick={() => handleSetExpiry(getDateFromDays(option.days))}
                >
                  {option.label}
                </Button>
              ))}
            ...
            {/* Manual Date Picker */}
            <div className="space-y-2">
              <label className="text-sm text-gray-600">Or enter a specific date:</label>
              <Input
                type="date"
        ...
        <Card className="overflow-hidden">
          <CardHeader className="bg-gradient-to-r from-emerald-50 to-green-50">
            ...
            <CardDescription>
              You&apos;ve reviewed all perishable items.
            </CardDescription>
          </CardHeader>
      ...
      {!isComplete && (
        <div className="flex flex-wrap gap-2">
          {itemsNeedingExpiry.map((item, index) => (
            ...
            >
              {item.name}
              {index < currentIndex && expiryDates[item.tempId] && (
